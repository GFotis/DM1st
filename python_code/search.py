...
def searching_vsm():
    text_query=""
    ttk.Entry(frm, width=50, textvariable=text_query).grid(column=0, row=0)
    vsm_query=knn(text_query,5)        
    resp = client.search(index="last_statement", query=vsm_query)
...
def construct_query2(choosen, fields, entries):
    ...
    must_not_text=None
    query ={
        "size": 4,#THELEI TO K POU SOU DINEI O XRHSTHS ANTI GIA 4
        "query": {
            "bool": {
                "must": [],
                "should": [],
    ...
    for i,field in enumerate(fields):
        if choosen[field]["choosen"].get():
            ...
            j+=1
    print(query)
    response = client.search(index="last_statement", body=query)

    # Print results
    for hit in response["hits"]["hits"]:
        print(f"Score: {hit['_score']}, Document: {hit['_source']['LastName']}")
